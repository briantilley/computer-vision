...
int DecoderGPU::sequence_callback(void *pUserData, CUVIDEOFORMAT* pVidFmt)
{
	// stitch C API and C++ together
	DecoderGPU* pInstance = reinterpret_cast<DecoderGPU*>(pUserData);
...
int DecoderGPU::decode_callback(void *pUserData, CUVIDPICPARAMS* pPicParams)
{
	// stitch C API and C++ together
	DecoderGPU* pInstance = reinterpret_cast<DecoderGPU*>(pUserData);
...
int DecoderGPU::output_callback(void *pUserData, CUVIDPARSERDISPINFO* pParDispInfo)
{
	// stitch C API and C++ together
	DecoderGPU* pInstance = reinterpret_cast<DecoderGPU*>(pUserData);
	...
	outputFrame = RawFrameGPU(devPtr, pitch, pInstance->m_width, pInstance->m_height, pInstance->m_width, (pInstance->m_height * 3 / 2), pParDispInfo->timestamp);
		// unmap output
	...
	if(pInstance->m_eosFlag)
	{
		...
		{
			pInstance->m_outputQueue.push(RawFrameGPU(true));
		}
	}
...
CUvideoctxlock DecoderGPU::s_lock;
bool DecoderGPU::s_lockInitialized = false;
int DecoderGPU::s_instanceCount = 0;

DecoderGPU::DecoderGPU(ConcurrentQueue<RawFrameGPU>& outputQueue): Decoder(outputQueue, true)
{
...
int DecoderGPU::decodeFrame(const CodedFrame& frame)
{
...
void DecoderGPU::signalEndOfStream(void)
{
	CUVIDSOURCEDATAPACKET sdp;
